...
from ..networks.contrastive_decoder import ContrastiveDecoder, MLP
...
class SkipConnectionMiddle(nn.Module):
    
    def __init__(self, submodule, dim: int = 1, mode = "cat") -> None:
        """
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        if self.mode == "cat":
            return torch.cat([x, y], dim=self.dim), y
        if self.mode == "add":
...
class CustomSequential(nn.Sequential):
    def __init__(self, *args, return_middle=False, buffer=[]):
        super().__init__(*args)
    ...
    def forward(self, input):
        ...
        for module in self:
            if i == 1:
                input, intermediate = module(input)
            else:
                input = module(input)
            i += 1
...
class UNetContrastive(nn.Module):
    ...
    def __init__(
        ...
        def _create_block(
            ...
            if len(channels) > 2:
                ...
                subblock = self._get_bottom_layer(c, channels[1])
                upc = c + channels[1]
                if contrastive_decoder_type == 'MLP':
                    self.c_decoder = MLP(input_dim=channels[1], output_dim=c_out_channels)
                else:    
                    self.c_decoder = ContrastiveDecoder(channels[1], c_out_channels)
                return_middle = True

            down = self._get_down_layer(inc, c, s, is_top)  # create layer in downsampling path
            up = self._get_up_layer(upc, outc, s, is_top)  # create layer in upsampling path

            return self._get_connection_block(down, up, subblock, return_middle=return_middle)

        self.model = _create_block(in_channels, out_channels, self.channels, self.strides, True)

    def _get_connection_block(self, down_path: nn.Module, up_path: nn.Module, subblock: nn.Module, return_middle) -> nn.Module:
        """
        Returns the block object defining a layer of the UNet structure including the implementation of the skip
        between encoding (down) and and decoding (up) sides of the network.
        Args:
        ...
        if return_middle:
            self.buffer = []
            return CustomSequential(down_path, SkipConnectionMiddle(subblock), up_path, return_middle=return_middle, buffer=self.buffer)
        return nn.Sequential(down_path, SkipConnection(subblock), up_path)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        self.buffer.clear()
        x = self.model(x)
        intermediate = self.buffer[0].flatten(start_dim=2, end_dim=-1).permute(0,2,1)
        x_c = self.c_decoder(intermediate)
        return x_c, x 
