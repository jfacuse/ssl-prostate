...
from monai.transforms import Compose, EnsureType, EnsureTyped
...
def prepare_datagens(args, fold_id, is_brats=False, transforms=[], seg_transforms=[], both_transforms=[], seed_for_shuffle=1):
    ...
    pretx = [EnsureType()] + transforms
    pretx_label = [EnsureType()] + seg_transforms
    pretx_both = [EnsureTyped(keys=['data', 'seg'])] + both_transforms
    check_ds = SimpleITKDataset(image_files=train_data[0][:args.batch_size*2],
                                ...
                                seg_transform=Compose(pretx_label),
                                both_transform=Compose(pretx_both),
                                is_brats=is_brats)
    check_loader = DataLoaderFromDataset(check_ds, batch_size=args.batch_size, num_threads=args.num_threads, seed_for_shuffle=seed_for_shuffle)
    ...
    train_ds = SimpleITKDataset(image_files=train_data[0], seg_files=train_data[1],
                                transform=Compose(pretx),  seg_transform=Compose(pretx_label), both_transform=Compose(pretx_both), is_brats=is_brats)
    valid_ds = SimpleITKDataset(image_files=valid_data[0], seg_files=valid_data[1],
                                transform=Compose(pretx),  seg_transform=Compose(pretx_label), both_transform=Compose(pretx_both), is_brats=is_brats)
    train_ldr = DataLoaderFromDataset(train_ds, 
        batch_size=args.batch_size, num_threads=args.num_threads, infinite=True, shuffle=True, seed_for_shuffle=seed_for_shuffle)
    valid_ldr = DataLoaderFromDataset(valid_ds, 
        batch_size=args.val_batch_size, num_threads=1, infinite=False, shuffle=False)

    return train_ldr, valid_ldr, class_weights.astype(np.float32)

def get_testing_datagen(args, transforms=[], seg_transforms=[], both_transforms=[], is_brats=False, test_file='PI-CAI_test.json'):
    # Abrir data sheet de testing
    with open(Path(args.overviews_dir) / test_file) as fp:
        test_json = json.load(fp)

    test_data = [np.array(test_json['image_paths']), np.array(test_json['label_paths'])]

    class_ratio_t = [int(np.sum(test_json['case_label'])), int(len(test_data[0])-np.sum(test_json['case_label']))]

    # log dataset definition
    print('Dataset Definition:', "-"*80)
    print('Data Classes:', list(np.unique(test_json['case_label'])))
    print(f'Testing Samples [-:{class_ratio_t[1]};+:{class_ratio_t[0]}]: {len(test_data[1])}')

    pretx = [EnsureType()] + transforms
    pretx_label = [EnsureType()] + seg_transforms
    pretx_both = [EnsureTyped(keys=['data', 'seg'])] + both_transforms
    test_ds = SimpleITKDataset(image_files=test_data[0], seg_files=test_data[1],
                                transform=Compose(pretx),  seg_transform=Compose(pretx_label), both_transform=Compose(pretx_both), is_brats=is_brats)
    test_ldr = DataLoaderFromDataset(test_ds, 
