...
class SimpleITKDataset(Dataset, Randomizable):
    ...
    def __getitem__(self, index: int):
        ...
        #print(img_hbv.shape)
        if self.is_brats:
            img_flair = z_score_norm(self.prepare_scan(str(self.image_files[index][3])), 99.5)
            img = np.concatenate([img_t2w, img_adc, img_hbv, img_flair], axis=1)
        else:
        ...
        if self.both_transform is not None:
            d = apply_transform(self.both_transform, {'data': img, 'seg': seg}, map_items=False)
            img = d['data']
            seg = d['seg']
