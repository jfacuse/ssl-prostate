...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(self, img_size=(32, 256, 256), patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True, in_chan_last=False):
        super().__init__()
        if len(img_size) == 1:
...
class VitContrastive(nn.Module):
    ...
    def __init__(self, encoder, decoder, args):
        ...
        self.decoder = decoder(in_channels=args.encoder_embed_dim,
    ...
    def forward(self, x_in, time_meters=None):
        ...
        s_time = time.perf_counter()
        x_contrastive = self.decoder(x)
        if time_meters is not None:
            torch.cuda.synchronize()
            time_meters['dec'].append(time.perf_counter() - s_time)
        return x_contrastive

class VitContrastiveReconstruct(nn.Module):
    """ Vit w/ contrastive output and reconstruct output
    """
    def __init__(self, encoder, decoder_c, decoder_r, args):
        super().__init__()
        if args.spatial_dim == 3:
            input_size = (args.roi_x, args.roi_y, args.roi_z)
        elif args.spatial_dim == 2:
        ...
        self.decoder_c = decoder_c(input_dim=args.encoder_embed_dim,
                               output_dim=args.out_channels)
        
        self.decoder_r = decoder_r(in_channels=args.in_chans,
                               out_channels=args.out_chans or args.num_classes,
                               img_size=input_size,
                               patch_size=args.patch_size,
    ...
    def forward(self, x_in, time_meters=None):
        ...
        s_time = time.perf_counter()
        x_contrastive = self.decoder_c(x)
        x_reconstruct = self.decoder_r(x_in, x, hidden_states)
        if time_meters is not None:
            torch.cuda.synchronize()
            time_meters['dec'].append(time.perf_counter() - s_time)
        return x_contrastive, x_reconstruct