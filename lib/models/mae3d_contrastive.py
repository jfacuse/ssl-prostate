...
class MAE3DContrastive(nn.Module):
    ...
    def __init__(self, 
        ...
        self.decoder_c = decoder_c(in_channels=args.decoder_embed_dim,
    ...
    def forward(self, x, return_image=False, return_embedding=False):
        args = self.args
        batch_size = x.size(0)
        in_chans = x.size(1)
        ...
        # forward decoder
        all_x_2 = self.decoder(all_x)

        # loss
        loss = self.criterion(input=all_x_2[:, -msk_length:, :], target=self.patch_norm(msk_x.detach()))

        embedding = self.decoder_c(all_x)
        ...
        if return_image:
            ...
            recon = all_x_2[:, 1:, :].gather(dim=1, index=unshuffle_indices[:, :, None].expand(-1, -1, out_chans))
